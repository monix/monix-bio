...
private[bio] object TaskExecuteOn {
  ...
  /** Implementation for `Task.executeOn`.
    */
  def apply[E, A](source: IO[E, A], s: Scheduler, forceAsync: Boolean): IO[E, A] = {
  ...
  private final class AsyncRegister[E, A](source: IO[E, A], s: Scheduler) extends ForkedRegister[E, A] {
  ...
  private final class TrampolinedStart[E, A](source: IO[E, A], s: Scheduler)
      extends ((Context[E], BiCallback[E, A]) => Unit) {

    def apply(ctx: Context[E], cb: BiCallback[E, A]): Unit = {
      val ctx2 = ctx.withScheduler(s)
      IO.unsafeStartNow(source, ctx2, cb)
    }
  }
}

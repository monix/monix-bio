...
trait TaskLift[F[_]] extends (Task.Unsafe ~> F) {

  /**
    * Converts `Task.Unsafe[A]` into `F[A]`.
    *
    ...
    */
  def apply[A](task: Task.Unsafe[A]): F[A]
...
object TaskLift extends TaskLiftImplicits0 {
  ...
  /**
    ...
    */
  implicit val toTask: TaskLift[Task.Unsafe] =
    new TaskLift[Task.Unsafe] {
      def apply[A](task: Task.Unsafe[A]): Task.Unsafe[A] = task
    }
  ...
  /**
    ...
    */
  implicit def toIO(implicit eff: ConcurrentEffect[Task.Unsafe]): TaskLift[IO] =
    new TaskLift[IO] {
      def apply[A](task: Task.Unsafe[A]): IO[A] = TaskConversions.toIO(task)(eff)
    }
...
private[bio] abstract class TaskLiftImplicits0 extends TaskLiftImplicits1 {
  ...
  /**
    ...
    */
  implicit def toConcurrent[F[_]](implicit F: Concurrent[F], eff: ConcurrentEffect[Task.Unsafe]): TaskLift[F] =
    new TaskLift[F] {
      def apply[A](task: Task.Unsafe[A]): F[A] = task.toConcurrent(F, eff, null)
    }
...
private[bio] abstract class TaskLiftImplicits1 extends TaskLiftImplicits2 {
  ...
  /**
    ...
    */
  implicit def toAsync[F[_]](implicit F: Async[F], eff: Effect[Task.Unsafe]): TaskLift[F] =
    new TaskLift[F] {
      def apply[A](task: Task.Unsafe[A]): F[A] = task.toAsync(F, eff, null)
    }
...
private[bio] abstract class TaskLiftImplicits2 {
  ...
  /**
    ...
    */
  implicit def toAnyLiftIO[F[_]](implicit F: LiftIO[F], eff: ConcurrentEffect[Task.Unsafe]): TaskLift[F] =
    new TaskLift[F] {
      def apply[A](task: Task.Unsafe[A]): F[A] = F.liftIO(TaskConversions.toIO(task)(eff))
    }
